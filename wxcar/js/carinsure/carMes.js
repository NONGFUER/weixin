...
var shengParamStr = ""; // 省级数据字符串，供安卓使用
var shiParamStr = ""; // 省级数据字符串，供安卓使用
...
$(function() {
	...
	userName = getUrlQueryString("mobile");
	fromtype = getUrlQueryString("fromtype");//渠道来源    第三方销售渠道接入同道车险
	if(userName!=null){
		parm = {
				"head":{
					"userName":userName,
		...
		str = UrlDecode(str);
		parm = JSON.parse(str)
		fromtype = parm.head.fromtype;//第三方销售渠道接入同道车险
	...
	if(!$.isNull(parm.body.inforCar) && parm.body.inforCar != undefined){
		...
		$.reqAjaxs(url, resData, function(data){
			if (data.statusCode == "000000") {
				...
				if(fromtype!=null){//第三方销售渠道接入同道车险
					$("#issueChannel").attr("channelCode",fromtype).val(changeChannel(fromtype));
				}
...
//					if(!$.isNull(data.returns.cxCarMessage.seats)){ //座位数
//					    $("#seat").val(parseInt(data.returns.cxCarMessage.seats).toFixed(0)).css("color","#585858");
//					}
//					if(!$.isNull(data.returns.cxCarMessage.vehicleBrand)){ //品牌型号jingyouVehicleName
//					    $("#brand_model_input").val(data.returns.cxCarMessage.vehicleBrand).css("color","#585858");
//					}
//					sessionStorage.setItem("tradeNo",data.returns.cxOrder.tradeNo);
//					sessionStorage.setItem("checkNo",data.returns.cxOrder.checkNo);
//					sessionStorage.setItem("checkCode",data.returns.cxOrder.checkCode);
//					if(data.returns.cxOrder.vehicleModelData!=null&&data.returns.cxOrder.vehicleModelData!=""){
//						vehicleModelData=JSON.parse(UrlDecode(data.returns.cxOrder.vehicleModelData)).vehicleModelData;
//					}
				}
			}else{
	...
	$(".h_back").unbind("tap").bind("tap",function() {
		...
		if (pageflag == 0) {
			...
			$(".backindex").hide();//隐藏返回按钮
			$(".mescontainer").show("fast");
			$("#plateIdNo").show("fast");
			$("#plateMobile").show("fast");
			$("#plateNumber").show("fast");
			$(".carimg").show();
			$(".mesdetail").hide();
			$(".litterhelp").hide();
			$(".imgIcon").show();
...
// 城市选择子页面消失时执行的回调
$.loadCityInfo = function(param) {
	param = eval("(" + param + ")");
	if (param != "goBack") {
		citynum = param.id;
		$("#car_shi").val(param.contName).css("width", "85%"); // 城市名称
		if (chooseicon == 0) {// 是否新车标志 1：新车，0：非新车
			$("#plate_number_input").val(param.cityPlate).css("color","#585858"); // 车牌号码开头
		}
	}
	;
};
...
function carinfoCheack() {
	...
	if ($.isNull(vehicle_identification_input)
			|| vehicle_identification_input == "请输入车辆识别代码") {
		carinfoCheakFlag = false;
		modelAlert("请输入车辆识别代码/VIN！");
		return false;
	} else if (vehicle_identification_input.length != 17) {
			...
			if (param.cityinfo.rows != null) {
				for ( var j = 0; j < param.cityinfo.rows.length; j++) {
					var customerinfo = new ProcessSheng(
							param.cityinfo.rows[j].contName, j);
