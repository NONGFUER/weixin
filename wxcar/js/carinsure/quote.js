...
$(function() {
	...
	$("#gouxuan").unbind("tap").bind("tap",function() {
		if($(this).attr("src")=="../../images/gouxuan2.png"){
			...
			$("#submitorder").css("background-color","#f36f39");
			gouxuan="1";
		}
		...
		if(cheakInsuredInfoFlag){//总校验通过
			...
			$.reqAjaxs(url, data,function(respData){
				if(respData.statusCode=="000000"){
					...
					window.location.href = "insuranceAgen.html?jsonStr=" + jsonStr;
				}else{
      ...
      if (param != null || param != "") {
		...
		if (param.status.statusCode == "000000") {
			if (param.cxInfo != null) {
				...
				var cxDistribution=parm.body.cxDistribution;
				if(cxDistribution!=null){//从配送地址列表页返回
					addressInfo.province = cxDistribution.province;
					addressInfo.address = cxDistribution.address;
					$("#sjrName").html(cxDistribution.receivername);
					$("#sjrPhone").html(cxDistribution.receiverphoneno);
					$("#addresseeprovince").html(cxDistribution.province.replace(new RegExp(" ","g"),""));
					$("#psAddress").html(cxDistribution.address);
					$(".addressInfo").show();
				}else{
