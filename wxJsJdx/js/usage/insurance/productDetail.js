...
function toInsure(){
	...
	if( roleType == "00" || roleType == "" ){
		window.location.href = base.url + "weixin/wxusers/html/users/phoneValidate.html?jsonKey="+jsonStr+"&fromtype=jsjdx&openid="+openid;
	}else{
...
function toCommodityList(){	
	...
	urlParm.downIco = "0"
	var jsonStr = UrlEncode(JSON.stringify(urlParm));
	window.location.href = base.url + "weixin/wxJsJdx/html/insurance/main/commodityList.html?jsonKey="+jsonStr;
}
//跳转到合同条款
function toHetongDemo(obj){
	...
	var jsonStr = UrlEncode(JSON.stringify(urlParm));
	window.location.href = base.url + "weixin/wxJsJdx/html/agreement/hetong.html?jsonKey="+jsonStr;
}
