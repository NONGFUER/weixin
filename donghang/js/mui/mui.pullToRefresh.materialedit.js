		...
		initPullDownTips: function() {
			...
			if ($.isFunction(self.options.down.callback)) {
				self.pullDownTips = (function() {
					...
					var mui_canvas = document.querySelector('canvas');
				    var mui_ctx = mui_canvas.getContext('2d');
				    var mui_ratio = getPixelRatio(mui_ctx);
					if (!element) {
						element = document.createElement('div');
						element.classList.add(CLASS_PULL_TOP_TIPS);
						element.innerHTML = '<div class="mui-pull-top-wrapper"><div class="mui-pull-top-canvas"><canvas id="pullDownTips" width="' + self.options.down.tips.size/mui_ratio + '" height="' + self.options.down.tips.size/mui_ratio + '"></canvas></div></div>';
