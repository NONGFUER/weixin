...
function updateData() {
	...
	// 取值
	gfbDistribution.id = $("#policyid").val();
	gfbDistribution.username = parm.head.userName;
	gfbDistribution.receivername = $("#recipient_name_input2").val(); // 收件人姓名
	gfbDistribution.receiverphoneno = $("#recipient_phone_input2").val(); // 收件人手机号
	gfbDistribution.address = $("#recipient_address_input2").val(); // 具体收件地址
	gfbDistribution.province = $("#recipient_city_input2").val();// 配送省份
	// 默认地址取值
	if ($(".switch_choose span:nth-of-type(1)").attr("class") == "policyholder_info_sex_selected") {
		gfbDistribution.defaultflag = "0"; // 非默认地址
	} else {
		gfbDistribution.defaultflag = "2"; // 默认地址
	};
	var data = {
