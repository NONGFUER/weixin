	...
	$("#submitorder").on("tap",function(){
		if(gouxuan=="1"){
			...
			if(cheakInsuredInfoFlag){//总校验通过
				...
				var url = base.url+'/dhController/dhOrderConfirm.do';
				var data = {
					'head':{
						'userCode': tradeNo,//暂存统一平台订单号
						'transTime':$.getTimeStr(),
						'channel':parm.head.source,  //来源
					},'body':{
						"addressInsuredDto":{
				...
				$.reqAjaxs(url, data,function(param){
					console.log(param)
					var dhtime=new Date().getTime();
					$(".detailcomfrim").hide();//投保按钮隐藏
					...
					if(param.statusCode=="000000"){
						window.location.href = base.dh+"s2i/confirm.html?randomno="+parm.head.userName+"&reason=&time="+dhtime;
					}else{
						var reason=encodeURI(param.statusMessage);
						window.location.href = base.dh+"s2i/confirm.html?randomno="+parm.head.userName+"&reason="+reason+"&time="+dhtime;
					}
			});
		  }
		}
