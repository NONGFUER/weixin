...
var base = {
		...
		url1 : window.location.protocol+"//"+window.location.host+"/tongdaoPlatform/weixin/ppStop/",
		...
		ppStopImagePath:window.location.protocol+"//"+window.location.host+"/tongdaoPlatform/weixin/ppStop/images/"
};
/**
 * @作者：马巍巍 时间：2016-01-13 功能：包含移动端所用的一些方法
 */
document.write("<script language='javascript' src='"+base.url1+"js/util/rollups/aes.js' ></script>");
document.write("<script language='javascript' src='"+base.url1+"js/util/components/pad-iso10126-min.js' ></script>");
document.write("<script language='javascript' src='"+base.url1+"js/util/weixin.js'></script>");
var dic_m = new Map();
var dic_m1 = new Map();
...
function modelAlert(message, describe, method) {
	...
	$("body").append('<div class="mui-popup mui-popup-in" style="display: block;"><div class="mui-popup-inner"><div class="mui-popup-title">温馨提示</div><div class="mui-popup-text">'+message+'</div></div><div class="mui-popup-buttons"><span class="mui-popup-button mui-popup-button-bold">'+describe+'</span></div></div>');
...
$.ajaxPrevent = function() {
	...
	ajaxPrevent += "<div class='ajax_prevent_a' style='width: 30%;margin-top: 55%;"
			...
			+ "color: #585858;'>";
	ajaxPrevent += "<img src='"+base.url1+"images/dh_loading.gif' style='width:100%'></div></div>";
...
/**----请求报文无request节点的ajax请求 是否同异步---*/
$.reqAjax = function(url, requestData,callBack,asyncStr) {
	if (asyncStr == ""||param==undefined) {
		asyncStr=true;
	}
	var requestJson = aesEncrypt(JSON.stringify(requestData), secretKey, secretKey);
	requestJson=URLencodeForBase64(requestJson);
...
$.replacePlaceholder = function(domObj, placeholderStr) {
	...
	$(domObj).unbind().bind({
		...
		blur : function() {
			if ($(domObj).val() == "") {
				$(domObj).val(placeholderStr).css("color", "#888");
				if (placeholderStr == "请输入密码") {
					domObj.attr("type", "text");
				}
			}
...
/**--返回pp停车微信公众号方法--*/
function goBackPPStop(issueChannel){
	if(issueChannel=="08"){
